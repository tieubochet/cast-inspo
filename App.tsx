...
const App: React.FC = () => {
  ...
  const fetchQuote = useCallback(async (quoteId?: string) => {
    ...
    } catch (error) { showToast("Failed to load quote.", "error"); } 
  ...
  const checkClaimStatus = useCallback(async (address: string) => {
    ...
    } catch (error) { console.error("Failed to check claim status:", error); }
  }, []);
  ...
  const checkNFTsStatus = useCallback(async (address: string) => {
    try {
      ...
      const dailyBalance = await publicClient.readContract({ 
        address: DAILY_NFT_CONTRACT, abi: ERC721_ABI, functionName: 'balanceOf', args: [address as `0x${string}`] 
      }) as bigint;
      setDailyQuoteCount(Number(dailyBalance));
    } catch (error) { console.error("Failed check NFTs:", error); }
  }, []);
  ...
  useEffect(() => {
    if (user?.fid) {
        getUserScore(user.fid).then(score => setRealScore(score));
    }
    refreshLeaderboard();
  ...
  // Update quote state when customizer changes it
  const handleQuoteUpdate = (updatedQuote: Quote) => { setCurrentQuote(updatedQuote); };
  ...
  const handleShare = async () => {
    ...
    try {
        ...
        try { imageUrl = await uploadQuoteImageToImgBB(currentQuote); } catch (e) { console.error(e); }
        const shareLink = generateSharingLink(currentQuote.id, imageUrl);
        if (toastIdRef.current) toast.dismiss(toastIdRef.current);
        sdk.actions.openUrl(shareLink);
    } catch (error) { showToast("Error preparing share.", "error"); } 
  ...
  const handleClaim = async () => {
    ...
    } catch (error: any) { showToast("Claim failed.", "error"); } 
  ...
  const handleMintGenesis = async () => {
    ...
    try {
      ...
      showToast("Minted Genesis! +500 PTS", "success");
    ...
    } catch (error: any) { showToast("Mint failed.", "error"); } 
    finally { setIsMintingGenesis(false); }
  };
  ...
  const handleMintDailyQuote = async () => {
    ...
    try {
        ...
        const metadata = {
            ...
            image: imageUrl,
            attributes: [{ trait_type: "Author", value: currentQuote.author }, { trait_type: "Source", value: "CastInspo" }]
        };
        ...
        await sdk.wallet.ethProvider.request({
            method: 'eth_sendTransaction',
            params: [{ to: DAILY_NFT_CONTRACT, from: userAddress, data: encodeFunctionData({ abi: ERC721_ABI, functionName: "mintQuote", args: [metadataURI] }) }]
        });
    ...
    finally { setIsMintingDaily(false); }
  };
  ...
  return (
    <div className="h-screen w-full bg-[#0a0a0a] text-white flex flex-col relative overflow-hidden font-sans selection:bg-purple-500/30">
      ...
      <div className="relative z-20 w-full flex-none">
        <Header user={user} onClaim={handleClaim} isClaiming={isClaiming} canClaim={canClaim} hasClaimedToday={hasClaimedToday} />
      </div>
        ...
        <main className="w-full min-h-full flex flex-col items-center px-4 pt-4 pb-28">
          {activeTab === Tab.HOME && (
              <QuoteCard 
                ...
                onShare={handleShare} 
                onQuoteUpdate={handleQuoteUpdate} // ENABLE CUSTOMIZER
              />
          )}
          {/* ... (Phần UI Mint và Reward giữ nguyên như trước) ... */}
          {activeTab === Tab.MINT && (
            <div className="flex flex-col items-center justify-start w-full animate-in fade-in slide-in-from-bottom-4 duration-500 space-y-8">
               <div className="w-full max-w-[400px] bg-zinc-900/60 backdrop-blur-md border border-zinc-800 rounded-3xl p-5 shadow-lg">
                   <div className="flex items-center gap-2 mb-4">
                       ...
                       <div className="flex flex-col"><h3 className="text-lg font-bold text-white leading-none">Quote of the Day</h3><span className="text-xs text-amber-300 font-medium">+20 Points per Mint</span></div>
                   </div>
                   <div className="aspect-[3/2] w-full bg-zinc-800 rounded-xl overflow-hidden mb-4 relative">
                        {currentQuote?.imageUrl ? <img src={currentQuote.imageUrl} alt="Quote" className="w-full h-full object-cover" /> : <div className="flex items-center justify-center h-full text-zinc-500 gap-2"><ImageIcon size={24} /><span className="text-xs">Preview loading...</span></div>}
                   </div>
                   <button onClick={handleMintDailyQuote} disabled={isMintingDaily || isLoading} className="w-full py-3 rounded-xl font-bold text-base bg-gradient-to-r from-amber-400 to-orange-500 text-black hover:opacity-90 active:scale-[0.98] transition-all flex items-center justify-center gap-2">
                      {isMintingDaily ? <Loader2 className="animate-spin" /> : <Sparkles size={18} />}{isMintingDaily ? "Minting..." : "Mint & Earn 20 PTS"}
                   </button>
               </div>
               <div className="w-full max-w-[400px] opacity-90 hover:opacity-100 transition-opacity">
                   <div className="flex items-center gap-2 mb-2 px-1"><Trophy className="text-purple-400" size={18} /><h3 className="text-sm font-bold text-zinc-300 uppercase tracking-wider">Genesis Collection</h3></div>
                   <div className="relative group w-full aspect-square rounded-3xl overflow-hidden shadow-2xl border border-white/10">
                        ...
                        <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-end p-6">
                            ...
                            <button onClick={handleMintGenesis} disabled={isMintingGenesis || hasGenesisNFT || genesisSupply >= 100} className={`w-full py-3 rounded-xl font-bold text-sm transition-all flex items-center justify-center gap-2 ${hasGenesisNFT ? 'bg-zinc-800 text-zinc-400 border border-zinc-700' : 'bg-white text-black hover:bg-zinc-200'}`}>
                                {isMintingGenesis ? <Loader2 className="animate-spin" size={16} /> : hasGenesisNFT ? <CheckCircle2 size={16} /> : "Mint Genesis"}{isMintingGenesis ? "Processing..." : hasGenesisNFT ? "Owned (+500 PTS)" : "Mint Free (+500 PTS)"}
                            </button>
                        </div>
                   </div>
               </div>
          ...
          {activeTab === Tab.REWARD && (
            <div className="flex flex-col items-center w-full animate-in fade-in slide-in-from-bottom-4 duration-500">
              <div className="w-full bg-zinc-900/60 backdrop-blur-md border border-zinc-800 rounded-3xl p-6 mb-6 shadow-xl relative overflow-hidden mt-2">
                 <div className="absolute top-0 right-0 p-4 opacity-10"><Trophy size={160} /></div>
                 <div className="relative z-10 flex flex-col gap-2">
                    <span className="text-zinc-400 text-xs font-bold uppercase tracking-wider">Your Total Score</span>
                    <div className="flex items-baseline gap-2"><h2 className="text-6xl font-bold text-white tracking-tighter">{realScore.toLocaleString()}</h2><span className="text-emerald-400 text-xl font-medium">PTS</span></div>
                    <div className="mt-4 flex items-center gap-3">
                        <div className="px-4 py-1.5 bg-purple-500/20 border border-purple-500/30 rounded-full text-purple-300 text-sm font-bold flex items-center gap-1.5 shadow-[0_0_15px_rgba(168,85,247,0.2)]"><Star size={14} fill="currentColor" />{userLevel}</div>
                    </div>
                 </div>
              </div>
                 ...
                 <div>
                    ...
                    <div className="space-y-3">
                        <div className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${hasClaimedToday ? 'bg-emerald-950/20 border-emerald-500/30' : 'bg-zinc-900/40 border-zinc-800'}`}>
                            <div className="flex items-center gap-3"><div className={`w-10 h-10 rounded-full flex items-center justify-center ${hasClaimedToday ? 'bg-emerald-500/20 text-emerald-400' : 'bg-zinc-800 text-zinc-500'}`}><Flame size={20} className={hasClaimedToday ? 'fill-current' : ''} /></div><div><p className="font-bold text-sm text-zinc-100">Daily Check-in</p><p className="text-xs text-zinc-500">+50 PTS</p></div></div>
                            {hasClaimedToday ? <CheckCircle2 size={20} className="text-emerald-500" /> : <button onClick={handleClaim} disabled={!canClaim || isClaiming} className="px-3 py-1.5 bg-white text-black text-xs font-bold rounded-lg hover:bg-zinc-200 disabled:opacity-50">{isClaiming ? '...' : canClaim ? 'Claim' : 'Locked'}</button>}
                        </div>
                        <div className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${dailyQuoteCount > 0 ? 'bg-amber-950/20 border-amber-500/30' : 'bg-zinc-900/40 border-zinc-800'}`}>
                             <div className="flex items-center gap-3"><div className={`w-10 h-10 rounded-full flex items-center justify-center ${dailyQuoteCount > 0 ? 'bg-amber-500/20 text-amber-400' : 'bg-zinc-800 text-zinc-500'}`}><Sparkles size={20} className={dailyQuoteCount > 0 ? 'fill-current' : ''} /></div><div><p className="font-bold text-sm text-zinc-100">Mint Quote</p><p className="text-xs text-zinc-500">+20 PTS {dailyQuoteCount > 0 && `(x${dailyQuoteCount})`}</p></div></div>
                        ...
                        <div className={`flex items-center justify-between p-4 rounded-2xl border transition-all ${hasGenesisNFT ? 'bg-purple-950/20 border-purple-500/30' : 'bg-zinc-900/40 border-zinc-800'}`}>
                             <div className="flex items-center gap-3"><div className={`w-10 h-10 rounded-full flex items-center justify-center ${hasGenesisNFT ? 'bg-purple-500/20 text-purple-400' : 'bg-zinc-800 text-zinc-500'}`}><Trophy size={20} className={hasGenesisNFT ? 'fill-current' : ''} /></div><div><p className="font-bold text-sm text-zinc-100">Genesis Badge</p><p className="text-xs text-zinc-500">+500 PTS</p></div></div>
                            {hasGenesisNFT ? <CheckCircle2 size={20} className="text-purple-500" /> : <button onClick={() => setActiveTab(Tab.MINT)} className="px-3 py-1.5 bg-zinc-800 text-white text-xs font-bold rounded-lg">Go</button>}
                        </div>
                    </div>
                 </div>
                 <div>
                     <h3 className="text-zinc-400 text-sm font-bold uppercase tracking-wider px-2 mb-2 flex items-center gap-2"><Crown size={14} className="text-amber-400" /> Global Leaderboard</h3>
                     <div className="bg-zinc-900/60 backdrop-blur-md border border-zinc-800 rounded-3xl overflow-hidden min-h-[200px]">
                        {leaderboardData.length === 0 ? (<div className="flex flex-col items-center justify-center p-8 text-zinc-500"><Loader2 className="animate-spin mb-2" /><span className="text-xs">Loading rankings...</span></div>) : (
                             leaderboardData.map((u, index) => {
                                 ...
                                 return (
                                    <div key={u.fid} className={`flex items-center justify-between p-4 border-b border-zinc-800/50 last:border-0 ${u.fid === user?.fid ? 'bg-purple-900/20 border-purple-500/30' : 'hover:bg-zinc-800/30'}`}>
                                        <div className="flex items-center gap-4"><div className={`w-6 text-center font-bold ${rank === 1 ? 'text-amber-400' : rank === 2 ? 'text-zinc-300' : rank === 3 ? 'text-amber-700' : 'text-zinc-600'}`}>{rank}</div>
                                            <div className="flex items-center gap-3"><img src={u.pfp_url || "https://picsum.photos/100/100"} className="w-8 h-8 rounded-full bg-zinc-800" alt="Avatar" /><span className={`font-medium ${rank <= 3 ? 'text-white' : 'text-zinc-400'}`}>{u.display_name || u.username || `FID: ${u.fid}`}{u.fid === user?.fid && " (You)"}</span></div>
                                        </div>
                                        <div className="font-mono text-sm font-bold text-emerald-400">{u.score.toLocaleString()}</div>
                                    </div>
                                 )
                             })
                        )}
      ...
      <div className="z-50 w-full"><Footer activeTab={activeTab} setActiveTab={setActiveTab} /></div>
      <ClaimSuccessModal isOpen={showSuccessModal} onClose={() => setShowSuccessModal(false)} onShare={() => sdk.actions.openUrl(`https://warpcast.com/~/compose?text=I%20just%20claimed%20rewards%20and%20leveled%20up%20on%20CastInspo!%20%F0%9F%94%A5`)} />
    </div>
  );
};
